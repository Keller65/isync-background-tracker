# Ingeniería y Historial de Refactorización (Sep 2026)

Notas internas de diseño y decisiones críticas del módulo. Documentación de uso
público: ver `README.md`.
...
## Unificación del contrato de la API (Sep 2026)

Antes del lanzamiento a npm se alineó la superficie pública con el contrato:

- `getMotionState()` ahora devuelve el shape `MotionSensorState` del contrato
  (`registered`, `moving`, `accelMagnitude`, `gyroMagnitude`, `magDelta`). Las
  magnitudes quedan en `0` porque el diseño usa solo Significant Motion. `registered`
  = trigger de hardware armado; `moving` = el Service no está estacionario.
- `getPendingPoints()` ahora incluye `sincronizado` e `intentos`.
- Default nativo de `interval` alineado a `5_000` (era `2_000`).
- Imports TS migrados de `expo` → `expo-modules-core` (el paquete solo declara ese
  dependency). `EventSubscription` → `Subscription`; `registerWebModule` → `createWebModule`.
- Plataformas reducidas a **solo Android** (se removió la superficie web y la apple).
- `LICENSE` atribuido a iSync (era el de 650 Industries/Expo).